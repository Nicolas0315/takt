...
fragment root の `params` は必須 facet parameter を宣言し、`uses` caller の `with` が全値を束縛する。型は `facet_ref` / `facet_ref[]`、kind は `policy` / `knowledge` / `instruction` / `report_format` のみで、default・optional・alias はない。`$param` は対応する facet field、`output_contracts.report[].format`、nested `uses` の明示的な `with` pass-through でのみ使える。nested fragment は outer param を暗黙 capture できない。`params` / `with` は展開時に消費され、caller overlay は parameter 展開後に適用される。
