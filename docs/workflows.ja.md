...
Finding Contract の修正ステップでは `team_leader.mode: finding_contract_fix` を指定できます。この mode は有効な `finding_contract` を必須とし、各 part を actionable finding へ明示的に割り当てます。assignment の `writePaths` と `readPaths` は作業ディレクトリからのリテラルな相対パスであり、ワイルドカードの `*` と `?` は使えません。`[]` などその他の文字は展開されず、パスの一部として扱われます。`writePaths` は並列作業の協調契約であり、filesystem sandbox ではありません。Team Leader は過去の raw 応答を累積せず、最新 batch 全体で上限を設けた raw excerpt・engine 検証済みの finding 単位 claim digest と、過去 batch における finding ごとの最新 digest から `continue`、`complete`、`replan` を判断します。`complete` にはステップ開始時の全 actionable finding を覆う `fixCoverage` と成功した検証が必要です。これは reviewer へ引き渡せるという step-local な判断であり、ledger の finding を解決するのは引き続き Finding Manager です。遷移は `when(structured.fix.decision == "complete")` のような機械条件で定義します。
