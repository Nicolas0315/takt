...
- **アップグレードで既存の `.takt/tasks.yaml` が壊れる問題を修正しました** (#1214)。v0.39.0 から v0.55.1 までのすべてのリリースは、タスクの開始ステップを `start_movement` というキーで書き出していました。#1207 がこのキーの読み書きを同時に削除したため、アップグレード後はこのキーを持つタスクが1件あるだけでファイル全体が検証に失敗し、`takt list` すら動かなくなっていました。旧キーを再び読んで `start_step` へ正規化します。書き込みは `start_step` のままなので、何かが保存された時点でファイルは移行されます。
- session-state のエンベロープ移行より前に記録されたタスクを再び読めるようにしました (#1191)。これらのタスクで「Mark as failed」が止まる問題が解消されます。
- OpenCode の構造イベント計数から content delta を除外しました (#1185)。ストリーミングされるテキストがガードの予算を消費しなくなります。
- OpenCode 隔離構造化実行の空応答を、空の `blocked` 結果ではなく error として表面化するようにしました (#1202)。あわせて Phase 2 ガードが OpenCode の構造化出力疑似ツールを不許可ツールとして誤拒否しないようにしました (#1203)。
- プロバイダからの `null` candidate を黙って通さず訂正機構へ載せ、それを許していたプロンプトの条件分岐漏れを直しました (#1204)。
