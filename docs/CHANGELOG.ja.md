...
- 従来からある「勝手な後方互換追加の禁止」を厳密化し、契約として検証される形にしました (#1172)。明示的な互換要求が効力を持つのは記載された対象とスコープのみで、期限付きの場合は期間と終了条件の明示が必須です。互換対象（スキーマ移行・データバックフィル・イベントアップキャスト・Read Model 再構築・API 互換）ごとに個別の根拠が必要になり、既存コンシューマーを新契約へ移行する作業はレガシーサポートではなく通常の置き換え作業として区別されます。既存コードの動作・既存テスト・保存済みデータ・リリース済みであることは影響分析の証拠であって、旧経路を残す根拠にはなりません。計画はサポート対象と除去契約を計画の出力契約に記録し、レビュアーは心得の文章ではなく契約として互換対応を検証します。
