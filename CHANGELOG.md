...
- Ctrl-C stays reliable during task runs (#1475, #1483). The run/watch interrupt handler re-resumes stdin when another component pauses it, so Ctrl-C keeps working through long concurrent runs, and Ctrl-C encoded as a kitty-keyboard-protocol (CSI-u) sequence is recognized as an interrupt as well.
