...
- **BREAKING:** The builtin TAKT development and Finding Contract workflow variants were consolidated (#1296). `takt-default-fc`, `takt-default-high`, `takt-default-team-high`, `takt-default-localllm`, `review-fix-takt-default-high`, and the Finding Contract builtin sub-workflows (`finding-contract-boundary-review`, `finding-contract-local-review`, `finding-contract-remediation`, `merge-readiness-finding-contract-final-gate`, `peer-review-finding-contract`, `peer-review-finding-contract-localllm`, `peer-review-suite-finding-contract-base`) were removed. TAKT development consolidates onto `takt-default` and the new `takt-experimental`, and the shared `development-core` now composes injectable `development-implement` / `development-remediation` subworkflows (plus `-dynamic` variants) with adjudication, verified remediation, follow-up review, and merge-readiness, parameterized by implementation facet pool and companions. The Finding Contract engine itself is unchanged and remains available to custom workflows via `finding_contract:`.
