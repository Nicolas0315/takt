...
| `npm run test:it` | Integration gate: parallel IT slice (`it-*`, integration/regression/performance suffixes, and explicitly classified legacy filenames), then the serial groups. |
| `npm test -- src/__tests__/<file>.test.ts` | Route a single file to the correct unit, parallel-IT, or serial-IT runner. Multiple routed runners execute concurrently and return the first failing exit code. |
...
| `npm run check:release` | Full pre-release gate: build + lint + fast 4-shard unit + test:it + prompt-evals + e2e. |

### Test pool behavior (worth knowing before "fixing" flakes)

- Legacy integration filenames and serial-IT membership live in `scripts/test-classification.mjs` (`parallelIntegrationTestFiles`, `serialGitTestFiles`, `serialWorkflowTestFiles`). Classification is based on process, Git, and workflow-engine boundaries; serial membership additionally requires measured IO interference. `releaseVerificationWiring.test.ts` fails if a listed file does not exist or overlaps another group.
